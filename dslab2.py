# 1.Recursive Python program for insertion sort
...
#2. Python 3 program to find numbers that are product of exactly two distinct prime numbers
...
#3. Recursive Python3 code to sort an array using selection sort Return minimum index
def minIndex( a , i , j ):
    if i == j:
        return i
...
#4. Python3 program for Recursive approach to find middle of singly linked list Node class
class Node:
...
# 5.Python3 implementation of the approach Recursive function to find the sum of even elements from the array
...
#6. Recursive approach to check if an Array is sorted or not  Function that returns 0 if a pair is found unsorted 
...
# 7.Print alternate nodes of a linked list using recursion	
class Node:
    def __init__(self, data):
       self.data = data
       self.next = None
...
#8. Python3 implementation to reverse a doubly linked list using recursion
import math
...
#9. Python function to print permutations of a given list
def permutation(lst):
...
# 10.Python Program for implementation of Recursive Bubble sort
class bubbleSort:
	"""
	bubbleSort:
		function:
